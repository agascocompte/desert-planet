...
import com.mygdx.game.animation.SilverCoinAnimation;
import com.mygdx.game.collision.CoinCollider;
import com.mygdx.game.collision.Collider;

public class SilverCoin extends GameEntity {

    private int score = 2;
    private Sound coinSound;

    public SilverCoin(int x, int y, int width, int height, String texturePath) {
        type = GameManager.ENTITY_TYPE.SILVER_COIN;
        setTexture(new Texture(texturePath));
        setPosition(x, y);
        setSize(width, height);
        animation = new SilverCoinAnimation(this);
        addCollider(new CoinCollider(this));

        coinSound = Gdx.audio.newSound(Gdx.files.internal("sound/effects/silver_coin.wav"));
